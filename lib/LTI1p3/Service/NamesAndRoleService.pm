...
sub getAllNamesAndRole {
	...
	foreach my $membership_request (@membership_requests) {
		my $members = $self->getNamesAndRole(
			$membership_request->{'client_id'},        $membership_request->{'context_id'},
			$membership_request->{'resource_link_id'}, $membership_request->{'context_memberships_url'},
		);
...
sub getNamesAndRole {
	...
	my $extralog = LTI1p3::ExtraLog->new($ce);
	$extralog->logNRPSRequest(
		"Beginning Names And Roles Service request for client: $client_id on context: $context_id with membership url: $context_memberships_url"
	);
	...
	while (1) {
		...
		if ($res->is_success) {
			...
			if ($res->content
				&& $res->content eq '{"errors":{"type":"bad_request","message":"Invalid \'rlid\' parameter"}}')
			{
			...
			$self->{error} =
				"Names And Roles Service request failed. "
				. "\nStatus: "
				. $res->status_line
				. "\nRequest URI: "
				. $res->request->uri

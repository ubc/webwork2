...
sub updateCourse {
	...
	while (my ($key, $val) = each(%userList)) {    # any users left in %userList has been dropped
		my $person = $val;
...
sub updateUser {
	...
	if (defined($newInfo->{'studentid'})
		&& $newInfo->{'studentid'} ne ''
		&& $newInfo->{'studentid'} ne $oldInfo->student_id())
	{
...
sub addUser {
	...
	if ($new_user_info->{'permission'}) {       # override default permission if necessary
...
sub addlog {
	...
	my $logfile = $self->{ce}->{lti_advantage}{studentlog};
	if ($logfile ne "") {
...
sub assignAllVisibleSetsToUser {
	...
	if (!$ce->{lti_advantage}{auto_assign_users_to_sets}) {
		return;
	}

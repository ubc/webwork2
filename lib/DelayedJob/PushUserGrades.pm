...
sub work {
	...
	$job->debug("Job: "
			. $job->jobid
			. ". Sending User LTI Assignment and Grades for course_id: $courseName user_id: $user_id set_id: $set_id");
	...
	if ($assignment_and_grade_service->{error}) {
		my $error_msg =
			"There was an issue pushing the user grades for course_id: $courseName user_id: $user_id set_id: $set_id. "
		...
		$job->debug("Job: "
				. $job->jobid
				. ". Successfully sent LTI Assignment and Grades for course_id: $courseName user_id: $user_id set_id: $set_id"
		);
		$job->completed();
	}
}

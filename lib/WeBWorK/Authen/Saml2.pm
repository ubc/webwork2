...
Still needs an internal Authen module like Basic_TheLastOption after this one
to handle already authed cookie sessions and for when the user wants to bypass
SAML2 auth.
...
sub request_has_data_for_this_verification_module($self) {
	...
	if ($bypassQuery && $c->param($bypassQuery)) { return 0; }
	# skip if there are signs of an existing user session
	# rpc calls like ajax problem rendering only has 'user' param
	my ($cookieUser, $cookieKey, $cookieTimeStamp) = $self->fetchCookie;
	if (defined $cookieUser || defined $c->param('user')) { return 0; }

...
sub get_credentials ($self) {
	...
	if (@errors) {
		...
		$self->{error} =
			"JSON Validation Errors:<br>" . join("<br>", map { "* [" . $_->{'path'} . "] " . $_->{'message'} } @errors);
		debug($self->{log_error});
		return 0;
	}
...
sub verifyIdToken ($self) {
	...
	if (!decode_jwt(token => $c->param("id_token"), kid_keys => $jwt_keys)) {
	...
	my $platform_id = $parser->get_param("iss");
	my $nonce       = $c->param("state") . $parser->get_param("nonce");
